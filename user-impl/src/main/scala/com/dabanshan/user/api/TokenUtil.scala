...
import com.dabanshan.commons.response.TokenContent
...
/**
  ...
  */
object TokenUtil {
  val secret = ConfigFactory.load().getString("dabanshan.secret")
  val authExpiration = ConfigFactory.load().getInt("dabanshan.token.auth.expirationInSeconds")
  val refreshExpiration = ConfigFactory.load().getInt("dabanshan.token.refresh.expirationInSeconds")
